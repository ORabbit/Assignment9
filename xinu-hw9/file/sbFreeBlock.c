...
devcall sbFreeBlock(struct superblock *psuper, int block)
{
	...
	// Case 2: Last collector node is full, must create a new one (with this block to be freed)... Case 3: There is a head, but no tail, must create tail
	if(freeblk->fr_count == FREEBLOCKMAX || (freeblk == psuper->sb_freelst && freeblk->fr_next == NULL))
	{
		if((newfreeblk = malloc(sizeof(struct freeblock))) == NULL)
			{signal(psuper->sb_freelock); return SYSERR;}
		newfreeblk->fr_blocknum = block;
		...
		if(SYSERR == write(diskfd, newfreeblk, sizeof(struct freeblock)))
			{signal(psuper->sb_freelock); return SYSERR;}
	}else // Case 4: Last collector node has room, increment count and add this block to the end
	{
		freeblk->fr_count++;
